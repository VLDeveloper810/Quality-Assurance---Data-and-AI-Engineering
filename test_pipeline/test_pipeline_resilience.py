...
def spark_fixture():
    ...
    spark = (
        SparkSession.builder.appName("Pytest-Pipeline-Data-Resilience-Testing")
        .master("local[2]")
...
def test_conflicting_financial_data_deduplication(spark_fixture):
    ...
    suffix_regex = (
        r"(?i)\b(inc|llc|corp|incorporated|group|co|ltd|international)\b|[.,_]"
    )
    ...
    mock_data = [
        (
            ...
            500000.0,
            120000.0,
        ),  # Row A: Has Address, Higher Revenue
        (
            "techcorp_id",
            "TechCorp",
            None,
            450000.0,
            110000.0,
        ),  # Row B: Missing Address, Lower Revenue
    ]
    ...
    ), "Data Resilience Failure: Duplicate corporate IDs were not collapsed!"
    ...
    assert (
        golden_record["address"] == "123 Innovation Way"
    ), "Deduplication Failure: Did not preserve valid address metadata."
    assert (
        golden_record["revenue"] == 500000.0
    ), "Deduplication Failure: Conflicting lower financial data was chosen."
...
def test_name_cleaning_edge_cases(spark_fixture):
    ...
    unique_ids = [
        row["corporate_id"]
        for row in df_cleaned.select("corporate_id").distinct().collect()
    ]

    # Assert that all 4 variations collapsed down to the exact same hash identifier
    assert (
        len(unique_ids) == 1
    ), f"Fuzzy Normalization Failure: Generated {len(unique_ids)} distinct IDs instead of 1."
