...
def generate_and_upload_datasets():
    ...
    for i in range(num_records):
        ...
        corp_name = (
            f"{base_name} {suffix}" if random.random() > 0.1 else f"{base_name}_{i}"
        )
        ...
        address = (
            ...
            else f"{random.randint(100, 999)} Main St, {CITIES[base_idx]}, {STATES[base_idx]}"
        )
    ...
    for i in range(num_records):
        ...
        corp_name = (
            f"{base_name} {alt_suffix}"
            if random.random() > 0.1
            else f"{base_name} International"
        )
