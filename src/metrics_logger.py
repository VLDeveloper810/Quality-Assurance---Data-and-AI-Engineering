...
class PipelineMonitor:
    ...
    def stop_timer(self, stage_name: str):
        ...
        if stage_name in self.start_times:
            ...
            logger.info(f"⏱️ [SLA TRACKING] Stage '{stage_name}' completed in {duration:.3f} seconds.")
            return duration
        return 0

    def detect_entity_drift(self, match_rate: float, historical_average: float = 65.0, threshold_deviation: float = 15.0):
        """
        ...
        logger.info(f"📊 [MONITOR] Current Match Rate: {match_rate:.2f}% (Historical Baseline: {historical_average}%)")
        ...
        if match_rate < lower_bound or match_rate > upper_bound:
            ...
            logger.info("✅ [HEALTH] Entity match rates fall within safe statistical boundaries.")
    ...
    def export_structured_health_dashboard(self, ml_metrics: dict = None):
        ...
        if ml_metrics:
            ...
            self.metrics["model_accuracy_auc"] = round(ml_metrics.get("accuracy", 0.0), 4)
            self.metrics["historical_baseline_accuracy"] = round(ml_metrics.get("baseline", 0.85), 4)
            
        dashboard = {
            "timestamp": datetime.utcnow().isoformat() + "Z",
            "pipeline_health_status": "CRITICAL" if self.metrics.get("entity_drift_triggered") else "HEALTHY",
